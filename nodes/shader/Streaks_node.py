...
class ShaderNodeStreaks(ShaderNode):
    ...
    def createNodetree(self, name):
        ...
        # Socket Ratio
        ratio_socket = nt.interface.new_socket(
            name="Ratio", in_out="INPUT", socket_type="NodeSocketFloat"
        )
        ratio_socket.default_value = 10.0
        ratio_socket.min_value = 9.999999747378752e-06
        ratio_socket.max_value = 3.4028234663852886e38
        ratio_socket.subtype = "NONE"
        ratio_socket.attribute_domain = "POINT"

        # Socket Count
        count_socket = nt.interface.new_socket(
            name="Count", in_out="INPUT", socket_type="NodeSocketFloat"
        )
        ...
        # Socket Variation
        variation_socket = nt.interface.new_socket(
            name="Variation", in_out="INPUT", socket_type="NodeSocketFloat"
        )
